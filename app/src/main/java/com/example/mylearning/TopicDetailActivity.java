...
import com.example.mylearning.databinding.ActivityTopicDetailBinding;
...
public class TopicDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ToolbarUtil.setup(this, binding.toolbar, topicName, true);
        ...
        binding.btnShortLesson.setOnClickListener(v -> {
    ...
    private void loadStats() {
        AsyncTask.execute(() -> {
            int accuracy = db.quizAttemptDao()
                    .getAccuracyForTopic(session.getUserId(), topicId);
            int attempts = db.quizAttemptDao()
                    .getAttemptCount(session.getUserId(), topicId);
            ...
            runOnUiThread(() -> {
                ...
                if (accuracy >= 70) {
                    binding.tvAccuracy.setTextColor(
                            ContextCompat.getColor(this, R.color.score_good_text));
                } else if (accuracy >= 50) {
                    binding.tvAccuracy.setTextColor(
                            ContextCompat.getColor(this, R.color.score_amber_text));
                } else {
                    binding.tvAccuracy.setTextColor(
                            ContextCompat.getColor(this, R.color.score_poor_text));
                }
            });
        });
    }
    ...
    private void loadAttempts() {
        AsyncTask.execute(() -> {
            List<QuizAttempt> attempts = db.quizAttemptDao()
                    .getAttemptsForTopic(session.getUserId(), topicId);
    ...
    private void renderAttempts(List<QuizAttempt> attempts) {
        ...
        for (QuizAttempt attempt : attempts) {
            ...
            tvTopicName.setText(dateFormat.format(new Date(attempt.timestamp)));
            tvDate.setText(attempt.score + " correct");
    ...
    private void handleAttemptTap(QuizAttempt attempt, LinearLayout containerQuestions,
        ...
        AsyncTask.execute(() -> {
            ...
            runOnUiThread(() -> {
                inflateQuestions(containerQuestions, questions);
                containerQuestions.setVisibility(View.VISIBLE);
            });
        });
    }

    // Full question cards — Explain, Star, and Video all functional
